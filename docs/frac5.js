...
class FracState {
    ...
    process1() {
        ...
        for (let pp = 0; pp < pts.length; pp += 5) {
            const x = Math.floor(
                (2.0 * Math.PI + pts[pp]) * (res - 1) / (4.0 * Math.PI))
            const y = Math.floor(
                (2.0 * Math.PI - pts[pp + 1]) * (res - 1) / (4.0 * Math.PI));
            let r = pts[pp + 2];
            let g = pts[pp + 3];
            let b = pts[pp + 4];
            ...
            this.imgBuff[bPosI] = Math.floor(
                127.5 * (1.0 - Math.cos(this.pixelSums[cPosI])));
            this.imgBuff[bPosI + 1] = Math.floor(
                127.5 * (1.0 - Math.cos(this.pixelSums[cPosI + 1])));
            this.imgBuff[bPosI + 2] = Math.floor(
                127.5 * (1.0 - Math.cos(this.pixelSums[cPosI + 2])));
...
function newState() {
    imageData = ctx.createImageData(cRes, cRes);
    const imageBuff = imageData.data;
    for (let i = 3; i < imageBuff.length; i += 4) {
        imageBuff[i] = 255;
    }
    const seqOps = makeSeqOps();
    return makeFracState(seqOps, txforms, cRes, imageBuff, zcolor());
}
